    ...
    var body: some View {
        List {
            Section {
                ...
                Text("支持 OpenAI 兼容接口与 Claude 官方接口，保存时会自动判断协议类型。")
            }
...
struct ModelEditView: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                Section("接口地址") {
                    TextField("请输入接口地址", text: $baseURL)
                        .keyboardType(.URL)
    ...
    private var inferredAPIType: ModelAPIType {
        let normalized = "\(baseURL) \(modelID) \(apiKey)".lowercased()
